...
";
            ...
            Assert.All(jobs, j => Assert.Equal(MessagingMode.Samples, j.MessagingMode));
            Assert.All(jobs, j => Assert.Single(j.DataSetWriters));
        }
...
";
            ...
            Assert.Equal(endpointUrls,
                jobs.Select(job => job.DataSetWriters
...
";
            ...
            enumerator.MoveNext();
            Assert.Single(enumerator.Current.DataSetWriters);
            enumerator.MoveNext();
            Assert.Equal(2, enumerator.Current.DataSetWriters.Count());
            Assert.Equal(endpointUrls,
                jobs.Select(job => job.DataSetWriters
                    .First().DataSet.DataSetSource.Connection.Endpoint.Url));
        }
...
");
            ...
            Assert.Single(jobs);
            Assert.All(jobs, j => Assert.Equal(MessagingMode.Samples, j.MessagingMode));
            Assert.Equal(10, jobs.Single().DataSetWriters.Count());
            Assert.All(jobs.Single().DataSetWriters, dataSetWriter => Assert.Equal("opc.tcp://localhost:50000",
                dataSetWriter.DataSet.DataSetSource.Connection.Endpoint.Url));
            Assert.All(jobs.Single().DataSetWriters, dataSetWriter => Assert.Null(
                dataSetWriter.DataSet.DataSetSource.SubscriptionSettings.PublishingInterval));
            Assert.All(jobs.Single().DataSetWriters, dataSetWriter => Assert.All(
                dataSetWriter.DataSet.DataSetSource.PublishedVariables.PublishedData,
                    p => Assert.Null(p.SamplingInterval)));
            Assert.All(jobs.Single().DataSetWriters, dataSetWriter =>
                Assert.Equal(1000,
...
");
            ...
            Assert.All(jobs.Single().DataSetWriters, dataSetWriter => Assert.Equal("opc.tcp://localhost:50000",
                dataSetWriter.DataSet.DataSetSource.Connection.Endpoint.Url));
            Assert.Equal(jobs.Single().DataSetWriters.Select(dataSetWriter =>
