...
namespace Microsoft.Azure.IIoT.OpcUa.Publisher.Storage.Default {
    ...
    public class WorkerDatabase : IWorkerRegistry, IWorkerRepository {
        ...
        public WorkerDatabase(IDatabaseServer server, IItemContainerConfig config) {
            var database = server.OpenAsync(config.DatabaseName).Result;
            var container = database.OpenContainerAsync(config.ContainerName).Result;
        ...
        /// <inheritdoc/>
        public async Task<WorkerInfoModel> GetWorkerAsync(string workerId,
            CancellationToken ct) {
