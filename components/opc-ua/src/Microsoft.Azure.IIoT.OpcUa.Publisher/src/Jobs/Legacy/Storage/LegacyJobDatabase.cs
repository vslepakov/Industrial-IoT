...
namespace Microsoft.Azure.IIoT.OpcUa.Publisher.Storage.Default {
    ...
    /// </summary>
    public class LegacyJobDatabase : ILegacyJobRepository {
        ...
        public LegacyJobDatabase(IDatabaseServer databaseServer, IItemContainerConfig databaseJobRepositoryConfig) {
        ...
        public async Task<JobInfoModel> AddAsync(JobInfoModel job, CancellationToken ct) {
            ...
            while (true) {
                var document = await _documents.FindAsync<LegacyJobDocument>(job.Id, ct);
                if (document != null) {
        ...
        public async Task<JobInfoModel> AddOrUpdateAsync(string jobId,
            ...
            while (true) {
                var document = await _documents.FindAsync<LegacyJobDocument>(jobId, ct);
        ...
        public async Task<JobInfoListModel> QueryAsync(JobInfoQueryModel query,
            ...
            var results = continuationToken != null ?
                client.Continue<LegacyJobDocument>(continuationToken, maxResults) :
                client.Query<LegacyJobDocument>(CreateQuery(query, out var queryParameters),
        ...
        public async Task<JobInfoModel> DeleteAsync(string jobId,
            ...
            while (true) {
                var document = await _documents.FindAsync<LegacyJobDocument>(
                    jobId);
                if (document == null) {
                    return null;
                }
        ...
        private static string CreateQuery(JobInfoQueryModel query,
            ...
            if (query?.Status != null) {
                queryString +=
$"r.{nameof(LegacyJobDocument.Status)} = @state AND ";
                queryParameters.Add("@state", query.Status.Value);
            }
            if (query?.Name != null) {
                queryString +=
$"r.{nameof(LegacyJobDocument.Name)} = @name AND ";
                queryParameters.Add("@name", query.Name);
            }
            queryString +=
$"r.{nameof(LegacyJobDocument.ClassType)} = '{LegacyJobDocument.ClassTypeName}'";
            return queryString;
        }
